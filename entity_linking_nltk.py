...
def findEntitiesInsentenceNLTK(sentence):
...
def refineDocument(document):
	...
	try:
       		for sentence in sentences:
        		entities_for_sentence = findEntitiesInSentenceNLTK(sentence)
...
def find_entities_freebase_id_nltk(named_entities,record_attribute):
        retval_list=[]
        for i in range (0,len(named_entities)):
                retval=[]
                #if named_entities[i][1]=='LOCATION' or named_entities[i][1]=='PERSON' or named_entities[i][1]=='ORGANIZATION':
                element=named_entities[i]
                f_id=find_entity_freebase_id(element)
                retval.append(record_attribute)
                retval.append(element)
                retval.append(f_id)
...
def find_entities(record):
	...
	retval= find_entities_freebase_id_nltk(named_entities,key)
        #print "retval_list is: ", retval
        yield retval

print "start-----------------------\n"
rdd_entities=rdd.flatMap(find_entities).saveAsTextFile(sys.argv[3])


#for x in rdd_entities.take(20):
 #       print x
